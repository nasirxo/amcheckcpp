...
bool CudaSpinSearcher::initialize() {
    ...
    config_.compute_capability = prop.major * 10 + prop.minor;
...
void CudaSpinSearcher::cleanup_device_memory() {
    ...
    if (d_symmetry_ops_) { cudaFree(d_symmetry_ops_); d_symmetry_ops_ = nullptr; }
    if (d_equiv_atoms_) { cudaFree(d_equiv_atoms_); d_equiv_atoms_ = nullptr; }
    if (d_spin_configs_) { cudaFree(d_spin_configs_); d_spin_configs_ = nullptr; }
